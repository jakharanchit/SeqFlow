...
const cli = join(repoRoot, 'bin', 'seqviz.mjs');
const fixture = join(repoRoot, 'fixtures', 'Sequence_XML.xml');
const work = mkdtempSync(join(tmpdir(), 'seqviz-cli-'));
...
function seqviz(...args: string[]): Run {
  try {
...
describe('emitting', () => {
  test('depth-2 writes the same bytes the app produces', () => {
    ...
    const run = seqviz(fixture, '--mode', 'depth-2', '--out', out);
  ...
  test('full and overview agree too', () => {
    expect(seqviz(fixture, '--stdout').stdout).toBe(toMermaid(graph, rules));
    expect(seqviz(fixture, '--mode', 'overview', '--stdout').stdout).toBe(
      toMermaid(graph, rules, { mode: { kind: 'overview' } }),
    );
  });

  test('--direction and --no-groups reach the emitter', () => {
    expect(seqviz(fixture, '--direction', 'LR', '--stdout').stdout).toBe(
      toMermaid(graph, rules, { direction: 'LR' }),
    );
    expect(seqviz(fixture, '--no-groups', '--stdout').stdout).toBe(
  ...
  test('split writes four files, matching the app', () => {
    ...
    expect(seqviz(fixture, '--mode', 'split', '--out', dir).status).toBe(0);
  ...
  test('a second run produces no diff', () => {
    ...
    seqviz(fixture, '--mode', 'depth-3', '--out', out);
...
describe('--check, the staleness guard', () => {
  ...
  test('exits 0 when the file on disk is current', () => {
    seqviz(fixture, '--mode', 'depth-2', '--out', out);
    expect(seqviz(fixture, '--mode', 'depth-2', '--out', out, '--check').status).toBe(0);
  });

  test('exits 1 and names the file when it is stale', () => {
    writeFileSync(out, 'flowchart TD\n  n1["nope"]\n');
    const run = seqviz(fixture, '--mode', 'depth-2', '--out', out, '--check');
  ...
  test('a missing file is stale, not a crash', () => {
    const run = seqviz(fixture, '--out', join(work, 'absent.mmd'), '--check');
    expect(run.status).toBe(1);
...
describe('what it writes', () => {
  test('it never writes sequence XML — invariant 4', () => {
    ...
    seqviz(fixture, '--mode', 'split', '--out', join(work, 'guard'));
