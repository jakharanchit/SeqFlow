...
function walk(
  ...
  while (queue.length > 0) {
    ...
    const outgoing = (direction === 'down' ? adj.out : adj.in).get(uid) ?? [];
 ...
 */
export function pathSet(graph: Graph, uid: string, adj = adjacency(graph)): PathSet {
  ...
  return { nodes, edges: graph.edges.filter((e) => seen.has(e)) };
}
